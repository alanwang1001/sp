...
// 它會自動過濾空白與註解，並辨識關鍵字（func, if, while, return）或識別碼（ID）。
// =========================================================
typedef enum {
...
void next_token() {
    ...
    if (isdigit(*src)) { 
        ...
        else if (strcmp(cur_token.text, "while") == 0) cur_token.type = TK_WHILE; // 支援 while 關鍵字
...
void statement() {
    ...
    // 加入 while 解析處理
        ...
        int loop_start = quad_count; // 記錄條件判斷的起始位址
        ...
        emit("JMP", start_str, "-", "-"); // 無條件跳回迴圈開頭
        
        sprintf(quads[jmp_f_idx].result, "%d", quad_count); // 回填跳出迴圈的地址
    } 
    else if (cur_token.type == TK_ID) {
        char name[32]; strcpy(name, cur_token.text); next_token();
...
void vm() {
    ...
    while (pc < quad_count) {
        ...
        else if (strcmp(q.op, "JMP") == 0) { // 新增無條件跳轉支援 while
            pc = atoi(q.arg1) - 1;
        }
